...
class GameController {
    constructor() {
        ...
        // 人机对战中，玩家侧可配置的规则参数
        this.aiHumanSettings = {
            forbiddenCount: 1,
            targetCount: 1,
            lockCount: 1,
            allowArithmeticOperators: false
        };
    ...
    initGame(rounds = 8, difficulty = 'normal', gameMode = 'local') {
        ...
        this.emit('gameInit', {
            ...
            targetCount: this.getEffectiveTargetCount(),
     ...
     */
    setAiHumanSetting(type, value) {
        if (!this.aiHumanSettings || !['forbiddenCount', 'targetCount', 'lockCount', 'allowArithmeticOperators'].includes(type)) {
            return false;
        }

        if (type === 'allowArithmeticOperators') {
            if (typeof value === 'boolean') {
                this.aiHumanSettings.allowArithmeticOperators = value;
            } else if (typeof value === 'string') {
                const normalized = value.toLowerCase();
                this.aiHumanSettings.allowArithmeticOperators = !['0', 'false', 'no', 'off', ''].includes(normalized);
            } else {
                this.aiHumanSettings.allowArithmeticOperators = Boolean(value);
            }
            return true;
        }
        ...
        if (type === 'forbiddenCount') {
            this.aiHumanSettings.forbiddenCount = Math.max(0, Math.min(20, parsed));
        } else if (type === 'targetCount') {
            this.aiHumanSettings.targetCount = Math.max(1, Math.min(6, parsed));
        } else if (type === 'lockCount') {
            this.aiHumanSettings.lockCount = Math.max(0, Math.min(20, parsed));
        }

        return true;
    }

    getAiTargetCountLimit() {
        return this.aiHumanSettings.targetCount;
    }

    getAiForbiddenCountLimit() {
        return this.aiHumanSettings.forbiddenCount;
    }

    getAiLockCountLimit() {
        return this.aiHumanSettings.lockCount;
    }

    getAiAllowArithmeticOperators() {
        return !!this.aiHumanSettings.allowArithmeticOperators;
    }

    shouldUseAiHumanSettings() {
        return this.gameMode === 'ai' && this.currentPlayer === 'A';
    }

    shouldAllowArithmeticOperators() {
        return this.shouldUseAiHumanSettings() && this.getAiAllowArithmeticOperators();
    }

    getEffectiveTargetCount() {
        if (this.shouldUseAiHumanSettings()) {
            return this.getAiTargetCountLimit();
        }
    ...
    canLockElement(element) {
        ...
        const isArithmeticOperator = ['+', '-', '*', '/'].includes(element);
        if (this.difficulty === 'easy' && isArithmeticOperator && !this.shouldAllowArithmeticOperators()) {
            return false;
        }
        return true;
    }
    ...
    getMaxForbiddenCount() {
        ...
        if (this.shouldUseAiHumanSettings()) {
            return this.getAiForbiddenCountLimit();
        }
    ...
    getMaxLockCount() {
        if (this.raceState && this.raceState.active) return 2;
        if (this.shouldUseAiHumanSettings()) {
            return this.getAiLockCountLimit();
        }
    ...
    selectTargetCell(cell) {
        ...
        // 检查是否已达到最大目标数
        if (this.roundState.targetCells.length >= this.getEffectiveTargetCount()) {
            // 替换最后一个选择的目标
        ...
        this.emit('targetSelected', { cell, count: this.roundState.targetCells.length, total: this.getEffectiveTargetCount() });
        return true;
    }
    ...
    evaluateResult(hitTargets, hitForbidden, functionType) {
        ...
        if (typeof hitTargets === 'boolean') {
            ...
            // 只有当所有目标格都被穿过时才算命中
            this.roundState.hitTarget = this.roundState.hitTargets.length >= this.getEffectiveTargetCount();
        }
    ...
    getGameState() {
        return {
            ...
            roundState: { ...this.roundState },
            maxForbidden: this.gameMode === 'ai' && this.currentPlayer === 'B' ? this.getAiForbiddenCountLimit() : this.getMaxForbiddenCount(),
            maxLocks: this.gameMode === 'ai' && this.currentPlayer === 'B' ? this.getAiLockCountLimit() : this.getMaxLockCount(),
