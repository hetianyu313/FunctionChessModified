...
class UIController {
    ...
    initUI() {
        ...
        // AI 人机对战规则悬浮菜单
        this.aiConfigMenu = null;
        this.aiConfigMenuBody = null;
        this.aiConfigMenuToggleBtn = null;
        this.aiConfigMenuArithmeticToggle = null;
        this.aiConfigMenuForbiddenInput = null;
        this.aiConfigMenuTargetInput = null;
        this.aiConfigMenuLockInput = null;
        this.aiConfigMenuForbiddenValue = null;
        this.aiConfigMenuTargetValue = null;
    ...
    initAiConfigFloatingMenu() {
        if (this.aiConfigMenu) return;

        const panel = document.createElement('div');
        panel.id = 'ai-config-floating-menu';
        panel.style.cssText = [
            'position: fixed',
            'right: 18px',
            'bottom: 18px',
        ...
        panel.innerHTML = `
            <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:8px; gap:8px;">
                <div style="font-weight: 700; color: #8dd9ff;">人机对战规则</div>
                <button id="ai-config-toggle-btn" type="button" style="border:none; border-radius:999px; padding:4px 8px; background:rgba(141,217,255,0.18); color:#8dd9ff; cursor:pointer; font-size:12px;">收起</button>
            </div>
            <div id="ai-config-menu-body" style="display:flex; flex-direction:column; gap:8px;">
                <label style="display:flex; flex-direction:column; gap:4px;">
                    <span style="display:flex; justify-content:space-between; align-items:center;">
                        <span>限制 AI 的禁区数量</span>
                        <strong id="ai-config-forbidden-value">1</strong>
                    </span>
                    <input id="ai-config-forbidden-input" type="range" min="0" max="20" step="1" value="1">
                </label>
                <label style="display:flex; flex-direction:column; gap:4px;">
                    <span style="display:flex; justify-content:space-between; align-items:center;">
                        <span>限制 AI 的目标区数量</span>
                        <strong id="ai-config-target-value">1</strong>
                    </span>
                    <input id="ai-config-target-input" type="range" min="1" max="6" step="1" value="1">
                </label>
                <label style="display:flex; flex-direction:column; gap:4px;">
                    <span style="display:flex; justify-content:space-between; align-items:center;">
                        <span>限制 AI 的数学元素数量</span>
                        <strong id="ai-config-lock-value">1</strong>
                    </span>
                    <input id="ai-config-lock-input" type="range" min="0" max="20" step="1" value="1">
                </label>
        ...
        this.aiConfigMenu = panel;
        this.aiConfigMenuBody = document.getElementById('ai-config-menu-body');
        this.aiConfigMenuToggleBtn = document.getElementById('ai-config-toggle-btn');
        this.aiConfigMenuArithmeticToggle = document.getElementById('ai-config-allow-arithmetic-input');
        this.aiConfigMenuForbiddenInput = document.getElementById('ai-config-forbidden-input');
        this.aiConfigMenuTargetInput = document.getElementById('ai-config-target-input');
        this.aiConfigMenuLockInput = document.getElementById('ai-config-lock-input');
        ...
        [this.aiConfigMenuForbiddenInput, this.aiConfigMenuTargetInput, this.aiConfigMenuLockInput].forEach(input => {
            if (!input) return;
            input.addEventListener('input', () => this.syncAiConfigFloatingMenu());
        });

        if (this.aiConfigMenuArithmeticToggle) {
            this.aiConfigMenuArithmeticToggle.addEventListener('change', () => this.syncAiConfigFloatingMenu());
        }
    ...
    syncAiConfigFloatingMenu() {
        if (!this.aiConfigMenu) return;

        const forbiddenValue = this.aiConfigMenuForbiddenInput ? Number(this.aiConfigMenuForbiddenInput.value) : this.gameController?.aiHumanSettings?.forbiddenCount || 1;
        const targetValue = this.aiConfigMenuTargetInput ? Number(this.aiConfigMenuTargetInput.value) : this.gameController?.aiHumanSettings?.targetCount || 1;
        const lockValue = this.aiConfigMenuLockInput ? Number(this.aiConfigMenuLockInput.value) : this.gameController?.aiHumanSettings?.lockCount || 1;
        const allowArithmeticValue = this.aiConfigMenuArithmeticToggle ? this.aiConfigMenuArithmeticToggle.checked : this.gameController?.getAiAllowArithmeticOperators?.() || false;

        if (this.aiConfigMenuForbiddenValue) this.aiConfigMenuForbiddenValue.textContent = `${forbiddenValue}`;
        if (this.aiConfigMenuTargetValue) this.aiConfigMenuTargetValue.textContent = `${targetValue}`;
        if (this.aiConfigMenuLockValue) this.aiConfigMenuLockValue.textContent = `${lockValue}`;
        if (this.aiConfigMenuArithmeticToggle) this.aiConfigMenuArithmeticToggle.checked = allowArithmeticValue;

        if (this.gameController?.setAiHumanSetting) {
            this.gameController.setAiHumanSetting('forbiddenCount', forbiddenValue);
            this.gameController.setAiHumanSetting('targetCount', targetValue);
            this.gameController.setAiHumanSetting('lockCount', lockValue);
            this.gameController.setAiHumanSetting('allowArithmeticOperators', allowArithmeticValue);
        }
    }

    toggleAiConfigFloatingMenu() {
        if (!this.aiConfigMenu) return;
        this.aiConfigMenuCollapsed = !this.aiConfigMenuCollapsed;
        this.applyAiConfigFloatingMenuCollapsedState();
    }

    applyAiConfigFloatingMenuCollapsedState() {
        if (!this.aiConfigMenu || !this.aiConfigMenuBody || !this.aiConfigMenuToggleBtn) return;
        this.aiConfigMenuBody.style.display = this.aiConfigMenuCollapsed ? 'none' : 'flex';
    ...
    initLockElementsView() {
        ...
        for (const element of allElements) {
            ...
            const allowArithmeticOperators = this.gameController?.shouldAllowArithmeticOperators?.() || false;
            const isProtectedInEasyMode = state.difficulty === 'easy' &&
                ['+', '-', '*', '/'].includes(element) && !allowArithmeticOperators;
    ...
    updatePhaseUI(phase) {
        ...
        switch (phase) {
            ...
            case 'set_forbidden':
                ...
                break;
            case 'set_locks':
                if (state.difficulty === 'easy' && !(this.gameController?.shouldAllowArithmeticOperators?.() || false)) {
